...
const generateAuthTokens = async (user) => {
  //Math.floor(Date.now() / 1000) gives the number of seconds to the current time
  const expiry =
    Math.floor(Date.now() / 1000) + config.jwt.accessExpirationMinutes * 60;

  const token = await generateToken(user._id, expiry, tokenTypes.ACCESS);

  return {
    access: {
      token: token,
      expires: new Date(expiry * 1000), //epoch time to standared date format
    },
  };
};
